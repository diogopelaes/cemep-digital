...
class AnoLetivoViewSet(GestaoWritePublicReadMixin, viewsets.ModelViewSet):
    ...
    def calendario(self, request, pk=None):
        ...
        ano = self.get_object()
        dias_nao_letivos = DiaNaoLetivoSerializer(ano.dias_nao_letivos.all(), many=True).data
        dias_extras = DiaLetivoExtraSerializer(ano.dias_letivos_extras.all(), many=True).data
    ...
    def remove_dia(self, request, pk=None):
        ...
        try:
            if tipo == 'nao_letivo':
                ...
                ano.dias_nao_letivos.remove(dia)
                ...
                ano.dias_letivos_extras.remove(dia)
                dia.delete()
            else:
