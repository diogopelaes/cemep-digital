...
import { HiPlus, HiTrash, HiBookOpen, HiX, HiCheck, HiAcademicCap, HiUpload, HiCheckCircle, HiXCircle, HiPencil } from 'react-icons/hi'
      ...
      >
        <div className="p-4">
          <div className="flex items-start justify-between gap-3">
            ...
            >
              <div className="w-10 h-10 shrink-0 rounded-xl bg-gradient-to-br from-accent-500 to-primary-500 flex items-center justify-center group-hover:scale-105 transition-transform shadow-sm">
                <HiBookOpen className="h-5 w-5 text-white" />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className={`font-bold text-sm group-hover:text-primary-600 transition-colors truncate ${!disciplina.is_active ? 'text-slate-400 line-through' : 'text-slate-800 dark:text-white'}`} title={disciplina.nome}>
                  {disciplina.nome?.length > 25 ? disciplina.nome.substring(0, 25) + '...' : disciplina.nome}
                </h3>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[11px] text-slate-500 font-mono bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">
                    {disciplina.sigla}
                  </span>
                  <span className="text-[10px] text-slate-400 bg-slate-50 dark:bg-slate-800/50 px-2 py-0.5 rounded-lg font-medium truncate max-w-[120px]">
            ...
            <button
              ...
              className={`
                  ...
                  : 'text-slate-400 hover:bg-slate-100 dark:text-slate-500 dark:hover:bg-slate-800'
                            ...
                            `}
            >
              {disciplina.is_active ? <HiCheckCircle className="h-5 w-5" /> : <HiXCircle className="h-5 w-5" />}
              <span>{disciplina.is_active ? 'Ativa' : 'Inativa'}</span>
            </button>
          </div>
        </div>
      </Card>
